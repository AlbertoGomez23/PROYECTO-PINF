C     Transformación de FENOLUN/BUSCA/IERA en FENOPLAN/BUSCAPLAN/IERAPLAN 
C     para utilizar su algoritmo para los planetas


C-----7---------------------------------------------------------------72-
C --- FENOPLAN proporciona la hora de un determinado fen—meno 
C        (orto, ocaso) paraun determinado Planeta
C
      FUNCTION FENOPLAN(dj,planet,fi,fen)
        IMPLICIT NONE
        LOGICAL ye
        CHARACTER*3 fen
        INTEGER sgn, planet
        REAL*8 dj, fi, FENOPLAN, salto, t0, dz0, ITERAPLAN, st
C
...
C
        ...
        st = 0.5D+00/24.D+00
C
C ---    buscaremos la hora del fen—meno
...
C
          CALL BUSCAPLAN(t0,st,dj,planet,fi,sgn,dz0)
C
...
C
            FENOPLAN = ITERAPLAN(t0,dj,planet,fi,dz0)
C
...
C        La condici—n ser’a 'FENOPLAN .LT. -0.5/24.'
C        En la condici—n escogemos '.LE.' puesto que la divisi—n no es
C        exacta.
C
            IF(FENOPLAN .LE. -8.333333333333D-03)THEN
...
C
      ...
      SUBROUTINE BUSCAPLAN(t0,st,dj,planet,fi,sgn,dz0)
C
C --- detecta el cambio de signo en la diferencia (dz - a0) entre
C        la distancia cenital del fen—meno (dz = dz0 al obviar paralajes y semidiámetro
C        y la (a0) correspondiente a una
...
C
        ...
        REAL*8 tt, TDBTDT, ar, de, ra, rr(6), dt, lonMad, gr2r
...
C
C
        CALL EQATORIA(planet,tt,ar,de,ra)
        dz = dz0 
C
...
C
        ...
        ha = MOD(dpi + MOD(ts0 - ar-lonMad*gr2r, dpi), dpi) 
C
...
C
      ...
      FUNCTION ITERAPLAN(t0,dj,planet,fi,dz0)
C
...
C
        ...
        REAL*8 DISTCE, eps, ret, rel, dz0, ITERAPLAN, dpi, OBLECL
        REAL*8 tt, TDBTDT, ar, de, ra, rr(6), gr2r, lonMad
        COMMON /decala/ dt 
        COMMON /dospi/ dpi ! ¹
        COMMON /retUA/ ret ! radio ecuatorial terrestre en UA
        COMMON /relUA/ rel ! radio ecuatorial lunar en UA
