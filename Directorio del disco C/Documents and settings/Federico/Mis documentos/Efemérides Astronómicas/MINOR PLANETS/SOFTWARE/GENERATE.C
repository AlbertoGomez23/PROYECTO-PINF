...
/* Generate a Chebyshev ephemeris file for an asteroid. */
	...
	double        **t, **tdot, time, tw[14][18], twt[14][14], **c1, **c2,
	              **c1c2, jd[9], **pv, **pv1, f, fdot, **coef, pos[3], vel[3],
	              jdt, *t_test, *td_test, testd, *sigma, test, len, jdinit,
	              jdfinal;
	static double bigbuf[513][7];
	int           k, j, i, l, biglen, order, numrec, startord;
	char          buf[21], infile[29], tmpfile1[30], tmpfile2[30],
	              outfile[30], hdrinfo[7];
...
/* Start main loop looking for best Chebyshev polynomial to represent a
   section of orbit.  Search is made by first increasing the order of the
   polynomial from 5 up to 13, and then decreasing the interval from 128 to 4
   days in factors of 2.  Search continues until all of the standard
   deviations are less than tol. */
...
/* Add the square of the difference between the ephemeris and computed
   position and velocity component to sigma. */
...
/* If the maximum standard deviation is less than the convergence tolerance
   ...
   the loop and print a warning message. */
				...
				if( test < *tol )
					...
					fprintf( stderr, "The segment beginning at JD " );
					fprintf( stderr, "%f did not converge.\n",
					         jd[8] + 2400000 );
					fprintf( stderr, "The convergence tolerance was " );
					fprintf( stderr, "%e and the ", *tol );
...
/* Add missing 240 0000 to initial Julian Date from PEP output, and set
   present final value for the covered segment. */
...
/* Move data from the temporary files to the permanent binary file. */
