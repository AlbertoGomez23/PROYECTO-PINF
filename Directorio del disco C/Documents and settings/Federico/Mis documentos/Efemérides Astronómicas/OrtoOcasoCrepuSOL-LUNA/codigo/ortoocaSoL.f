...
c        de un determinado fen—meno de Sol (crepœsculos, orto, ...)
      ...
      FUNCTION FENOSOL(dj,fi,fen)
        ...
        CHARACTER*3 fen
        INTEGER sgn, dt, j
        REAL*8 gr2r, dj, fi, dz, FENOSOL, ts0, tdb0, u0, u1, u2, a1
        REAL*8 ha, ANGPOL, TSMUT, a0, DISTCE, eps, c5, dpi, BUSCASOL
        REAL*8 rr(6), ar, de, ra, tt, TDBTDT, OBLECL
        ...
        DATA j/11/
        SELECT CASE(fen)
          CASE('pca') ! principio crepœsculo astronómico
            sgn = -1
            dz = 108.*gr2r
          ...
          CASE('fcn') ! fin crepœsculo n‡utico
            sgn = 1
            dz = 102.*gr2r
          CASE('fca') ! fin crepœsculo astronómico
            sgn = 1
            dz = 108.*gr2r
        END SELECT
C 
...
c  habremos detectado la hora del fen—meno y saldremos de FENOSOL.
        ...
        REAL*8 tt, TDBTDT, ar, de, ra, OBLECL
        COMMON/decala/ dt 
        COMMON /dospi/ dpi ! ¹
        REAL*8 cpi
        COMMON /ctePI/ cpi ! ¹
