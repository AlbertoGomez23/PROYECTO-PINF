...
%%%%%%%%%%%%%%%%%%%%%%%%%%% T\'{I}TULO
\title{Una presentaci\'{o}n sencilla con «beamer»}

\author[J. L. Varona]{Juan Luis Varona\\ \texttt{jvarona@unirioja.es}}

\institute[U. de La Rioja]{Universidad de La Rioja}

\date{20 de noviembre de 2007}
...
\subsection{Material gr\'{a}fico}

\begin{frame}

\frametitle{La sexta pantalla}

Se incluyen gr\'{a}ficos sin problemas:

\only<2->{
\begin{itemize}
\item Fotos.

\only<3->{\item Dibujos hechos con otro programa.}

\only<4->{\item Generados con c\'{o}digo \LaTeX.}

\only<5->{\item Im\'{a}genes con fondo transparente
(compara el \texttt{jpg} con el \texttt{png}).}
\end{itemize}}
...
\onslide<1>
  \begin{center}
  \includegraphics[scale=0.9]{./Figuras/s1.pdf}
  \end{center}
\onslide<2>
% Plot[Sin[x]/x, {x, -13, 13}, PlotRange -> All,
%   PlotStyle -> {RGBColor[1, 0, 0], Thickness[0.01]}]
  \begin{center}
  \includegraphics[scale=0.9]{./Figuras/s2.pdf}
  \end{center}
%\transdissolve<3>
...
\draw (0,0) circle (1cm); % circunferencia, centrada en (0,0)
\draw (0,0) +(0:1cm) node[right]{$G$} -- +(30:1cm) node[right]{$I$}
  -- +(60:1cm) node[above right]{$Q$} -- +(90:1cm) node[above]{$B$}
  -- +(120:1cm) node[above left]{$H$} -- +(150:1cm) node[left]{$Z$}
  -- +(180:1cm) node[left]{$A$} -- +(210:1cm) node[left]{$N$}
  -- +(240:1cm) node[below left]{$M$} -- +(270:1cm) node[below]{$D$}
  -- +(300:1cm) node[below right]{$L$} -- +(330:1cm) node[right]{$K$}
  -- +(360:1cm) -- cycle ; % pol\'{\i}gono (dodec\'{a}gono)
\draw (0,0) +(0:1cm) -- +(180:1cm); % eje X
\draw (0,0) +(30:1cm) -- +(-30:1cm); % l\'{\i}neas verticales
\draw (0,0) +(60:1cm) -- +(-60:1cm); % ..
\draw (0,0) +(90:1cm) -- +(270:1cm);
\draw (0,0) +(120:1cm) -- +(240:1cm);
\draw (0,0) +(150:1cm) -- +(210:1cm);
\draw (0,0) +(30:1cm) -- +(210:1cm); % l\'{\i}neas oblicuas
\draw (0,0) +(60:1cm) -- +(240:1cm); % ...
\draw (0,0) +(120:1cm) -- +(300:1cm);
\draw (0,0) +(150:1cm) -- +(330:1cm);
% El \'{u}ltimo, para que los dem\'{a}s no "machaquen" el color "red!10" de fondo
% (el mismo fondo que he asignado en \setbeamertemplate{background canvas}):
%\draw (0,0) node[fill=red!10, above right=1.5pt]{$X$};
% (no queda muy bien, pues no hay manera de conseguir el degradado).
% As\'{\i} que mejor lo pongo sin fondo:
\draw (0,0) node[above right=1.5pt]{$X$};
\end{tikzpicture}
\end{center}
\onslide<5>
\begin{center}
\includegraphics[width=0.4\textwidth]{./Figuras/s3.pdf}
\qquad\qquad
\includegraphics[width=0.4\textwidth]{./Figuras/s4.pdf}\\
{\small El formato \texttt{jpg} (izquierda) no admite transparencias;
el \texttt{png} (derecha) s\'{\i}.}
\end{center}
\end{overprint}
