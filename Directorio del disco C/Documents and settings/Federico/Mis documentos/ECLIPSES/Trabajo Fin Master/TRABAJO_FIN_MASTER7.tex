...
\par
Desde tiempos remotos el hombre se ha sentido atra\'{\i}do por los fen\'{o}menos astron\'{o}micos, siendo el del eclipse de Sol el que probablemente m\'{a}s impresi\'{o}n, asombro, supersticiones, etc., ha generado. Este fen\'{o}meno va m\'{a}s all\'{a} de la simple contemplaci\'{o}n, ya que las conclusiones obtenidas de su observaci\'on, han servido a veces para descubrir, y otras para corroborar, conocimientos claves en la comprensi\'{o}n de diversas teor\'{\i}as cient\'{\i}ficas. Es por ello que a lo largo de la historia y conforme los conocimientos de las civilizaciones avanzaban, se ha intentado conocer, cada vez con m\'{a}s precisi\'{o}n, el cu\'{a}ndo, en qu\'{e} circunstancias y desde d\'{o}nde, se pod\'{\i}an observar estos acontecimientos.
\par
Los caldeos ya conoc\'{\i}an desde la antig\"{u}edad el periodo Saros, ciclo de repetici\'{o}n de eclipses cada 18 a\~{n}os y 11 d\'{\i}as. Pero es a finales del siglo XIX, cuando ya se tienen conocimientos y herramientas suficientes para predecir, con bastante precisi\'{o}n, la ocurrencia de eclipses. Sin embargo los m\'{e}todos tradicionales no son en absoluto sencillos. Se han utilizado m\'etodos anal\'{\i}ticos que resuelven parte del problema; estos m\'etodos se basan en una serie de transformaciones de variables, que, adem\'{a}s de introducir simplificaciones que implican p\'erdida de precisi\'{o}n, hacen perder la noci\'{o}n geom\'{e}trica de qu\'{e} se est\'a calculando. Pero los procedimientos num\'ericos han sido desde siempre los empleados en el c\'alculo de eclipses. Basados en algoritmos iterativos, permiten calcular los distintos elementos de un eclipse partiendo de un dato aproximado, que se refina en pocas iteraciones; como datos iniciales sol\'{\i}an tomarse una hora aproximada, calculada adem\'as para una Tierra esf\'erica, hora que se hac\'{\i}a converger a la real, introduciendo en algunos casos el achatamiento terrestre. La aparici\'{o}n de las computadoras automatiz\'{o} completamente estos procedimientos, desarroll\'{a}ndose programas (sobre todo en FORTRAN) que realizaban esta tarea.

La aparici\'{o}n relativamente reciente de procesadores simb\'{o}licos (como \textsl{Mathematica} aqu\'{\i} utilizado), invita a plantear y resolver las ecuaciones que determinan los par\'{a}metros caracter\'{\i}sticos de los eclipses de una forma mucho m\'{a}s sencilla y clara que hasta ahora. Si bien la precisi\'{o}n obtenida no mejora la actual, y las propias funciones internas llevan un mecanismo parecido en cuanto a iteraciones etc, a los programas ya existentes, el hecho de poder plantear la resoluci\'{o}n de los distintos problemas mediante sencillas y elegantes f\'{o}rmulas, supone un novedoso enfoque del problema.
\par

El Real Instituto y Observatorio de la Armada (ROA) publica desde 1791 las \textit{Efem\'{e}rides Astron\'{o}micas}, siendo esta publicaci\'on el cuarto de los almanaques de car\'acter nacional que aparecieron en el mundo; desde entonces, las efem\'erides se han publicado anualmente sin interrupci\'on, con las t\'ecnicas disponibles en cada momento. Aunque, como hemos dicho, los c\'{a}lculos realizados con estos programas no van a aportar mayor precisi\'{o}n a los datos que actualmente se publican, s\'{\i} servir\'an para cotejar los resultados obtenidos mediante los medios actuales, para automatizar la representaci\'{o}n de los mapas de eclipses, y, por \'{u}ltimo, para ayudar a la docencia, facilitando la explicaci\'{o}n sencilla y clara de este fen\'{o}meno.
\par
...
Mientras que los eclipses de Luna son visibles para todos los puntos de la superficie terrestre que tengan en ese momento a la Luna por encima del horizonte, los de Sol s\'olo ser\'an visibles para algunas zonas de la Tierra.
...
Como ya se ha comentado, los eclipses de Sol son visibles \'unicamente desde determinadas zonas de la superficie terrestre, variando su desarrollo de una localizaci\'on a otra. Por tanto, es necesario dar la informaci\'on suficiente para que los interesados puedan conocer, no s\'olo la fecha y el tipo de eclipse, sino en qu\'e lugares y en qu\'e condiciones se desarrollar\'a \'este. Tradicionalmente, esta informaci\'on consta de lo siguiente:
\begin{itemize}
...
Los eclipses de Sol ocurren aproximadamente cuando el Sol y la Luna tienen la misma ascensi\'on recta. Esta hora se toma como referencia para facilitar al usuario los datos que necesita para calcular c\'omo se desarrollar\'a el eclipse en una localizaci\'on concreta, es decir, las circunstancias locales del eclipse. Aunque actualmente el c\'alculo de las circunstancias locales se realiza mediante los elementos besselianos, las publicaciones especializadas siguen incluyendo los siguientes datos en el instante de la conjunci\'on en ascensi\'on recta:
\begin{itemize}
  \item Hora de la conjunci\'{o}n.
  \item Declinaci\'{o}n y ascensi\'{o}n recta de Sol y Luna.
...
\begin{itemize}
  ...
  \item \textit{Principio y fin del eclipse total/anular}. An\'{a}logo al anterior, pero con el cono de sombra en lugar del de penumbra. Los puntos correspondientes de la Tierra son el primero y el \'{u}ltimo que ven los limbos del Sol y la Luna como tangentes interiores.
  ...
  Estos puntos suelen existir para la mayor\'{\i}a de los eclipses \textit{centrales}.
...
Curvas l\'{\i}mite superior e inferior (esencialmente Norte y Sur) sobre la Tierra, desde donde se puede ver el eclipse.

\item[Curvas l\'{\i}mite Norte y Sur de la sombra.]
Curvas l\'{\i}mite superior e inferior desde donde se puede ver el eclipse como total/anular.
...
La posici\'on geogr\'afica de un observador viene dada por su longitud $\lambda$, su latitud geod\'esica $\varphi$, definidas de la forma habitual, y su altura $H$ sobre el elipsoide de referencia. Para obtener sus coordenadas geogr\'aficas cartesianas, es necesario calcular la latitud geoc\'entrica $\phi$, cuya relaci\'on con la geod\'esica viene dada por:
\[
\begin{array}{r@{{}={}}l}
...
\begin{itemize}
  ...
  \item Centros geom\'{e}tricos de Sol y Tierra coincidiendo con el centro de masas. Desviaci\'{o}n del centro geom\'{e}trico de la Luna respecto de su centro de masas el indicado por IAU (1976).
  \item Se considera que el eclipse est\'{a} en el horizonte, es decir que se est\'{a} produciendo al orto o al ocaso, cuando el punto de observaci\'{o}n tiene nula su componente $\zeta$. Esto supone despreciar la refracci\'{o}n atmosf\'{e}rica y considerar, para estos puntos, el cono como un cilindro de la misma base. Si se quisiese ser m\'{a}s preciso habr\'{\i}a que estudiar las circunstancias locales del eclipse.
\end{itemize}
...
A partir de estas efem\'erides se prepara una peque\~na base de datos que cubre el per\'{\i}odo de desarrollo del eclipse. En concreto, se generan dos ficheros (uno para el Sol y otro para la Luna) que contienen las coordenadas requeridas, tabuladas a intervalos regulares de Tiempo Terrestre (TT).
...
Con los valores discretos de los elementos besselianos a lo largo de todo el per\'{\i}odo, mediante  las funciones \verb"Interpolation" y \verb"Derivative" de \textsl{Mathematica}, se crean funciones interpolantes y sus derivadas, que ser\'{a}n utilizadas a lo largo de todo el programa.

\subsection{C\'{a}lculo de la distancia $q$ del eje de la sombra a la elipse fundamental}
El semieje menor $b$ de la elipse fundamental se calcula a partir del elemento $d$, para cada instante tabulado. Su valor, en unidades del radio ecuatorial terrestre, es el siguiente: \begin{equation}\label{b}
b = \sqrt{\cos^2 u+(1-f)^2\sen^2 u},
\end{equation}
...
Si $\min(q-l_p)<0$, se calcula el valor de $\min(q-l_s)$. Si \'este es $>0$, el eclipse es \textit{parcial}, y en caso contrario, es total/anular; la discusi\'on de si es total, anular o mixto se realiza en el apartado \ref{s:tipoanular}. En estos \'ultimos casos, si $\min(q)>0$ el eclipse es \textit{no central}, siendo central en caso contrario.
...
La distancia del eje de la sombra al centro de la Tierra es $\sqrt{x^2+y^2}$. El instante en que esta distancia es m\'inima lo obtenemos mediante la funci\'{o}n \verb"FindRoot" aplicada a la derivada de dicha expresi\'{o}n.

Para obtener el punto de m\'{a}ximo del eclipse distinguimos dos casos, seg\'{u}n el eclipse sea central o no.
...
\subsection{C\'{a}lculo de los puntos de coincidencia de las curvas de m\'{a}ximo en el horizonte con las curvas l\'{\i}mite  Norte/Sur}
...
\begin{figure}[p]
\centering
\includegraphics[height=\textwidth,angle=90]{./Figuras/26ene2009.pdf}
\caption[Representaci\'{o}n eclipse central con las dos curvas de parcialidad]{Representaci\'{o}n de eclipse central con las dos curvas de parcialidad. Eclipse anular de 26 de enero de 2009.}\label{tipoE}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[height=\textwidth,angle=90]{./Figuras/01ago2008.pdf}
\caption[Representaci\'{o}n eclipse central con una sola curva de parcialidad]{Representaci\'{o}n de eclipse central con una sola curva de parcialidad. Eclipse total de 1 de agosto de 2008.}\label{tipoD}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[height=\textwidth,angle=90]{./Figuras/31may2003.pdf}
\caption[Representaci\'{o}n eclipse central con una sola curva de totalidad]{Representaci\'{o}n de eclipse central con una sola curva de totalidad. Eclipse anular de 31 de mayo de 2003.}\label{tipoC}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[height=\textwidth,angle=90]{./Figuras/19abr2004.pdf}
\caption[Representaci\'{o}n eclipse parcial]{Representaci\'{o}n de eclipse parcial. Eclipse parcial de 19 de abril de 2004.}\label{tipoA}
\end{figure}

En las figuras \ref{tipoE}, \ref{tipoD}, \ref{tipoC} y \ref{tipoA} se muestran ejemplos de los mapas generados por el programa, para distintos tipos de eclipses. Dos de ellos corresponden a los mapas de las figuras \ref{ejemploanular} y \ref{ejemploparcial} que se utilizaron para describir las diferentes curvas.
...
%\begin{figure}[p]
%\centering
%\includegraphics[width=\textwidth]{./Figuras/11jul2010c5c1.pdf}
%\caption[Representaci\'{o}n eclipse sin c\'{a}lculo puntos %coincidencia curvas de m\'{a}ximo en el horizonte con curvas l\'{\i}%mite Norte/Sur penumbra]{Representaci\'{o}n eclipse sin c\'{a}lculo %puntos coincidencia curvas de m\'{a}ximo en el horizonte con %curvas l\'{\i}mite Norte/Sur penumbra, con datos cada cinco %minutos (arriba) y con datos cada minuto (abajo). Eclipse de 11 de %julio de 2010}\label{c5c1}
%\end{figure}
\par
Los procesadores simb\'olicos, especialmente \textsl{Mathematica}, son una excelente herramienta a la hora de dibujar los mapas de los eclipses.
\par
Debemos destacar el estudio detallado del desarrollo del eclipse en zonas cr\'{\i}ticas, que ha concluido en el descubrimiento de una nueva curva l\'{\i}mite, de cuya existencia no se tiene constancia en la bibliograf\'{\i}a consultada. La utilidad de esta nueva curva es muy cuestionable desde el punto de vista pr\'{a}ctico. \'{E}sta aparece en regiones polares, donde la presencia humana es escasa y adem\'{a}s su situaci\'{o}n difiere muy poco de las curvas pr\'{o}ximas. A todo esto hay que a\~{n}adir el hecho de que el eclipse en esos puntos se ve en el horizonte, con las imprecisiones que ello conlleva debido a la refracci\'{o}n atmosf\'{e}rica. Sin embargo, desde el punto de vista conceptual y acad\'emico s\'{\i} es importante, ya que permite comprender la geometr\'{\i}a del eclipse en esas zonas cr\'{\i}ticas.
...
Son el conjunto de funciones creadas en \textsl{Mathematica} para la ejecuci\'{o}n de todos los algoritmos necesarios.
...
\item{\bf ascensi\'{o}n recta}: distancia angular medida hacia el E sobre el
