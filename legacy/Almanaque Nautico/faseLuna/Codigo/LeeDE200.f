C-----7---------------------------------------------------------------72-
      SUBROUTINE ABRDE200(u) ! Å EPHOPN; OPENS u ='JPLEPH'
        INTEGER u
!        CALL STATE(0.D0,0,0,0) ! inicializa
      END


      SUBROUTINE CIERRADE(u) ! CLOSES u = 'JPLEPH'
        INTEGER u
        CLOSE(u)
        RETURN
      END
      ...
      SUBROUTINE LEEEFJPL(JD,TARG,CENT,RRD,INSIDE)
        IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        SAVE ! RRD = AU & AU/DAY. FOR NUTATION, RADIANS & RADIANS/DAY.
        REAL*8 JD, RRD(6) ! CALLING SEQUENCE DECLARATIONS
        INTEGER TARG, CENT
        LOGICAL INSIDE ! MISCELLANEOUS DECLARATIONS
        CALL PLEPH (JD,TARG,CENT,RRD)
        RETURN
      END
...
C          (SEE THE DISCUSSION IN THE SUBROUTINE STATE)
  ...
  11    DO I = 1, 6
        ...
        END DO
        IF(FIRST)THEN
          CALL STATE(0.D0,0,0.D0,0.D0) !lee IPT
          FIRST = .FALSE.
        END IF
        ...
        DO I = 1, 12
          LIST(I) = 0
        END DO
        IF(NTARG .NE. 14) GO TO 97 ! CHECK FOR NUTATION CALL
        IF(IPT(35) .GT. 0)THEN
          LIST(11) = 2
  ...
  297     FORMAT(' *****  NO NUTATIONS ON THE EPHEMERIS FILE  *****')
          STOP
        END IF
  97    IF(NTARG .NE. 15) GO TO 98 ! CHECK FOR LIBRATIONS
        IF(IPT(38) .GT. 0)THEN
          LIST(12) = 2
          CALL STATE(ET2,LIST,PV,RRD)
          DO I = 1, 6
  ...
  298     FORMAT(' *****  NO LIBRATIONS ON THE EPHEMERIS FILE  *****')
          STOP
        END IF
  98    BSAVE = BARY ! FORCE BARYCENTRIC OUTPUT BY 'STATE'
        BARY = .TRUE.
C       SET UP PROPER ENTRIES IN 'LIST' ARRAY FOR STATE CALL
        DO I = 1, 2 ! SET UP PROPER ENTRIES IN 'LIST' ARRAY FOR STATE CALL 
C      Da el valor list(k)=2, a aquellos planetas/Luna involucrados (Tierra-Luna van juntos)
          K = NTARG
          IF(I .EQ. 2) K = NCENT
          IF(K .LE. 10) LIST(K) = 2
          IF(K .EQ. 10) LIST(3) = 2
          ...
          IF(K .EQ. 13) LIST(3) = 2
        END DO
        CALL STATE(ET2,LIST,PV,RRD) ! MAKE CALL TO STATE (está en baricéntrico)
	  IF(NTARG .EQ. 11 .OR. NCENT .EQ. 11)THEN
          DO I = 1, 6
            PV(I,11) = PVSUN(I) !(le da al Sol sus valores)
          END DO
        END IF
        IF(NTARG .EQ. 12 .OR. NCENT .EQ. 12)THEN
          DO I = 1, 6
            PV(I,12) = 0.D0 !(le da al baricentroSS los valores nulos)
          END DO
        END IF
        IF(NTARG .EQ. 13 .OR. NCENT .EQ. 13)THEN
          DO I = 1, 6
            PV(I,13) = PV(I,3) 
C        el PV(I,3) de STATE es el del baricentro Tierra-Luna
          END DO
        END IF
        IF(NTARG*NCENT .EQ. 30 .AND. NTARG+NCENT .EQ. 13)THEN
          DO I = 1, 6
            PV(I,3) = 0.D0 !(si Tierra y bariT-L a Tierra le da valor nulo)
          END DO
          GO TO 99 !esto no lo entiendo (ejem NTARG=3 y NCENT=13)
        END IF
        IF(LIST(3) .EQ. 2)THEN
          DO I=1,6
            PV(I,3) = PV(I,3) - PV(I,10)/(1.D0 + EMRAT)
C        la Tierra es el baricentro menos la Luna respecto al baricentro escalado por las masas
          END DO
        END IF
        IF(LIST(10) .EQ. 2)THEN
          DO I = 1, 6
            PV(I,10) = PV(I,3) + PV(I,10)
C        la Luna es el baricentro + la Luna respecto al baricentro
          END DO
        END IF
  99    DO I = 1, 6
...
C     CALLING SEQUENCE PARAMETERS:
C       INPUT:
C         BUF  1ST LOCATION OF ARRAY OF D.P. CHEBYSHEV COEF. OF POSITION
...
C  AND ARE STORED IN THE ARRAY 'PC'.
        IF(NP .LT. NCF)THEN
          DO 1 I = NP+1, NCF
            PC(I) = TWOT*PC(I-1) - PC(I-2)!fórmula de recurrencia
    1     CONTINUE
          NP = NCF
        END IF
C  INTERPOLATE TO GET POSITION FOR EACH COMPONENT
        DO 2 I = 1, NCM !en cada coordenada
          PV(I,1) = 0.D0
          DO 3 J = NCF, 1, -1
            PV(I,1) = PV(I,1) + PC(J)*BUF(J,I,L)!multiplico pol por coeficiente
    3     CONTINUE
    2   CONTINUE
        IF(IFL .LE. 1) RETURN
...
C  DERIVATIVE POLYNOMIALS HAVE BEEN GENERATED AND STORED.
        ...
        IF(NV .LT. NCF)THEN
          DO 4 I = NV + 1, NCF
            VC(I) = TWOT*VC(I-1) + PC(I-1) + PC(I-1) - VC(I-2)!recurrencia
    4     CONTINUE
          NV = NCF
        END IF
C  INTERPOLATE TO GET VELOCITY FOR EACH COMPONENT
        DO 5 I = 1, NCM !en cada coordenada
          PV(I,2) = 0.D0
          DO 6 J = NCF, 2, -1
            PV(I,2) = PV(I,2) + VC(J)*BUF(J,I,L)!multiplico poli por coeficiente
    6     CONTINUE
          PV(I,2) = PV(I,2)*VFAC !escalado por ser tiempo normalizado 
    5   CONTINUE
        RETURN
      END
...
C++++++++++++++++++++++++++++++++
C  READS AND INTERPOLATES THE JPL PLANETARY EPHEMERIS FILE
C    CALLING SEQUENCE PARAMETERS:
C    INPUT:
...
C               ET2(1) = SOME FIXED EPOCH, SUCH AS START OF
C               INTEGRATION, AND ET2(2) = ELAPSED INTERVAL BETWEEN
C               THEN AND EPOCH.
...
C            DEFAULT VALUE = .FALSE.  (KM DETERMINES TIME UNIT FOR
C              NUTATIONS AND LIBRATIONS. ANGLE UNIT IS ALWAYS RADIANS.)
...
C             VELOCITY OF THE SUN.
     ...
     .          SS(3), CVAL(400), PVSUN(6)
      ...
      CHARACTER*120 NAMFIL
      LOGICAL KM, BARY
      COMMON /EPHHDR/ CVAL, SS, AU, EMRAT, NUMDE, NCON, IPT
      COMMON /STCOMX/ KM, BARY, PVSUN
C      se salta COMMON/CHRHDR/CNAM, TTL e incluye COMMON /logiunit
      COMMON /logiunit/ NRFILE, NCOEFFS
      LOGICAL FIRST
      DATA FIRST /.TRUE./ !(creo que esto ya sobra)
C  ENTRY POINT - 1ST TIME IN, GET POINTER DATA, ETC., FROM EPH FILE
      IF(FIRST)THEN
        FIRST = .FALSE.


C   AÑADIDO MIO SEGUN TESTEPH     
...
C   FIN AÑADIDO
     ...
     &       FORM = 'UNFORMATTED',
     &       RECL = IRECSZ,
     &       READONLY, ! No se por quŽ la necesita
     &       STATUS = 'OLD')
...
C       ********** MAIN ENTRY POINT **********                                  
        IF(ET2(1) .EQ. 0.D0) RETURN ! la primera llamada desde PLEPH   
  	  S = ET2(1) - 0.5D0
        CALL SPLIT(S,PJD(1))
        CALL SPLIT(ET2(2),PJD(3))
        PJD(1) = PJD(1) + PJD(3) + 0.5D0
        PJD(2) = PJD(2) + PJD(4)
        CALL SPLIT(PJD(2),PJD(3))
        PJD(1) = PJD(1) + PJD(3)
C  ERROR RETURN FOR EPOCH OUT OF RANGE
        IF(PJD(1) + PJD(4).LT.SS(1) .OR. PJD(1) + PJD(4).GT.SS(2))
     .    GO TO 98
...
C  READ CORRECT RECORD IF NOT IN CORE (esta versión lo lee siempre)
...
C      INICIO VARIACIONES HECHAS 	                                                                          
      DO I=1,6                                                                  
        PVSUN(I)=PVSUN(I)*AUFAC   	                         
      ENDDO                                                                     
C       FIN VARIACIONES HECHAS
...
C   SUBRUTINA AÑADIDA PARA EVITAR EL ARCHIVO 'DON'T TOUCH ME'
...
C  NAMFIL IS THE EXTERNAL NAME OF THE BINARY EPHEMERIS FILE                     
      ...
      NAMFIL='C:\Program Files\Microsoft Visual Studio\Common\MSDEV98\
     +My Projects\ALMANAQUE NAUTICO\FASES LUNA\JPLEPH'                                                           
...
c  aquí le he dado valor para de405
c                                                                                
      KSIZE=2036                                                                  
